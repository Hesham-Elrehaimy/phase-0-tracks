...
#define a variable and assign an empty array in which real names will be stored to it 
real_names = []
#define a variable and assign an empty array in which fake names will be stored to it 
fake_names = []
...
#add the user's imput to the real names array
...
p code_name
#add the user's input to the fake names array
...
#remove the word "done" from the real names array
real_names.delete('done')

#initialize the index counter at 0
i = 0
#create a while loop which is executed as long as the index couter is smaller than the value of the length of the real names array
while 
    i < real_names.length
#print a message that tells the real name of the person with the fake name    
    puts "#{fake_names[i]} is actualy #{real_names[i]}."
